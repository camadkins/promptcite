...
## Why PromptCite isn't gameable

When the student runs `/receipt` **inside the AI they're disclosing**
(e.g. typing `/receipt` in a Claude Code session to disclose Claude use),
PromptCite asks the AI itself to fill in the tool name, model, and date —
not the student. The receipt records this as `metadata_source: "agent_reported"`.

That field is the moat: it lets an instructor see, at a glance, whether
the tool/model/date came from the AI itself (verifiable, the student
can't lie about which AI they used) or from the student's recollection
of a previous session (`metadata_source: "student_claimed"`). A receipt
generated inside Claude with `tool: "Claude"` and `metadata_source:
"agent_reported"` is the same kind of evidence as a citation
auto-generated by a reference manager — the source attribution didn't
go through the student's keyboard.

The content fields (what the student asked the AI, what they changed
or kept, whether they verified sources) are always student-authored
regardless. PromptCite cannot stop a student from lying about the
content of their use — and it doesn't try to. The trust model is the
same as citations: the author writes them, the reader evaluates them,
and the artifact is a transparency record, not a lie detector. But on
the metadata fields where verification IS possible — which AI, which
model, which day — PromptCite gets that for free when the disclosure
happens in-session.
